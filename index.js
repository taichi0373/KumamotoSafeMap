// const url_comments = "https://uemura-taichi.com/server/server_comments.php";
const url_comments = "./server/server_comments.php";
  ...
  fetch(url_comments, {
    ...
    .then(($jsonResponse) => {
      if ($jsonResponse.status === "success") {
        const coord = $jsonResponse.get_location;
        var accident_latitude = coord[0].latitude;
        var accident_longitude = coord[0].longitude;
        near_markers = findComment(accident_latitude, accident_longitude, accident_id)
              ...
              for (const coord of coordinates) {
                ...
                if (loggedInUsername === coord.username) {
                  comment_popup.innerHTML +=
                    ...
                    "</button>" +
                    // 編集フォームの戻るボタン
                    `<button class="btn btn-dark btn-sm ms-1" type="button" id="return-button-${comment_id}" style="width:40px; height:24px; margin:0px; padding:0px; display:none; font-size:12px;" onclick="Return(${accident_id})">` +
                    "戻る" +
                    "</button>" +
                    // 編集フォームの変更ボタン
                    `<button class="btn btn-primary btn-sm ms-1" type="button" id="edit-button-${comment_id}" value=${comment_id} style="width:60px; height:24px; margin:0px; padding:0px; display:none; font-size:12px;" onclick="editComment(${comment_id}, ${accident_id})">` +
                    "変更する" +
                    "</button>" +
                    "</p>";
                }
...
function findComment(accident_latitude, accident_longitude, id) {
  const distances = [];
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  button_show.addEventListener("click", function () {
    ...
    for (const checkbox of checkboxes) {
      if (checkbox.checked) {
        check_element2 = true;
        const layerNumber = parseInt(checkbox.getAttribute("layer-number"));
      ...
      for (const selected_year of selected_years) {
        ...
        worker.onmessage = function (event) {
          if (event.data.status == "success") {
            ...
            const markerCoordinate = `${latitude}_${longitude}_${event.data.year}`;

            // 既に同じ座標のマーカーが追加されていないか確認
            if (!addedMarkerCoordinates.has(markerCoordinate)) {
              addedMarkerCoordinates.add(markerCoordinate);
      ...
      for (const selected_year_sm of selected_years_sm) {
        ...
        worker.onmessage = function (event) {
          if (event.data.status == "success") {
            ...
            const markerCoordinate_sm = `${latitude_sm}_${longitude_sm}_${event.data.year}`;

            // 既に同じ座標のマーカーが追加されていないか確認
            if (!addedMarkerCoordinates_sm.has(markerCoordinate_sm)) {
              addedMarkerCoordinates_sm.add(markerCoordinate_sm);
